...
func _ready():
	add_to_group(Global.GROUP_NPC_ID)

func _process(delta):
	if Player_is_in_FOV_TOLERANCE() and Player_is_in_LOS():
		$Torch.color = RED
		get_tree().call_group(Global.GROUP_SUSPICION_METER_ID, "player_seen")
	else:
		$Torch.color = WHITE
...
func Player_is_in_LOS():
	...
	var LOS_obstacle = space.intersect_ray(global_position, Player.global_position, [self], get_collision_mask())
